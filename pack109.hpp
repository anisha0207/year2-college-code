...
#define PACK109_STATUS_TAG 0x01 

//kept same-- no changes
...
namespace pack109 {
  ...
  vec serialize(const std::map<std::string, vec>& m);
  std::map<std::string, vec> deserialize_map_vec_u8(const vec& bytes);
